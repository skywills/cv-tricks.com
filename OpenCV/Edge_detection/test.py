...
start = time.time()
gray, edges = edge_detection(out,resize_scale=1, sigma=2.9, l_thresh=0.1, h_thresh=0.29)
image1 = border_detection(image,edges)
print(time.time() - start)
#con=np.concatenate((image,gray),axis=1)
cv2.imshow('used {}'.format(time.time() - start),out)
